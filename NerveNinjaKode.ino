...
void pulse(int start, int end, int step) {
  for (int i = start; i != end; i += step) {
    strip.setPixelColor(i, strip.Color(100, 100, 100));
    strip.setPixelColor(i - step, strip.Color(0, 0, 0));
    strip.show();
    delay(10);
  }
  strip.setPixelColor(end - step, strip.Color(0, 0, 0));
  strip.show();
}

void nerveSignalHAG() { pulse(12, 2, -1); }
void nerveSignalHBG() { pulse(36, 46, 1); }
void nerveSignalHG() { pulse(54, 50, -1); }
void nerveSignalVBG() { pulse(13, 22, 1); }
void nerveSignalVAG() { pulse(36, 26, -1); }
